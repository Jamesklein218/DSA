...
public:
    ...
    bool remove(Node * & p, const T &value) {
        ...
        if (p->data == value) {
            if (p->pLeft && p->pRight) {
                Node *pT = p->pLeft;
                while (pT->pRight) pT = pT->pRight;
                p->data = pT->data;
                if (remove(p->pLeft, p->data)) return !balanceRight(p);
            } else if (p->pLeft) {
                Node *pT = p;
                p = p->pLeft;
                delete pT;
            ...
            if (remove(p->pLeft, value)) return !balanceRight(p);
        } else
            if (remove(p->pRight, value)) return !balanceLeft(p);
